...
public class GoodsServiceImpl implements GoodsService {
	...
	public void add(Goods goods) {
		...
		goodsDescMapper.insert(goods.getGoodsDesc());  // 插入商品信息
	...
	private void setItemValus(Goods goods, TbItem item) {
		...
		item.setCategoryid(goods.getGoods().getCategory1Id()); // 商品分类编号 ps:最末级
	...
	public void updateMarketable(Long[] ids, String marketable) {
		for (Long id : ids) {
			...
			if ("1".equals(goods.getAuditStatus())) {
				...
				// System.out.println(marketable+"   这是状态           "+id);
				goods.setIsMarketable(marketable);
				goodsMapper.updateByPrimaryKey(goods);
			}else {
				throw new RuntimeException("只有通过审核的商品才能进行上架操作！");
			}
		}
